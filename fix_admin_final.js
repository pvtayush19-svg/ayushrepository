...
async function fixAdminAgain() {
  try {
    console.log('1. Dropping problematic auto-confirm trigger...');
    await executeSql("DROP TRIGGER IF EXISTS on_auth_user_created_confirm ON auth.users; DROP FUNCTION IF EXISTS public.auto_confirm_email;");

    console.log('2. Deleting old admin user via SQL...');
    await executeSql("DELETE FROM auth.users WHERE email = 'admin@medocare.com'");
    
    console.log('3. Signing up admin user via Supabase API...');
    ...
    console.log('4. Elevating role to admin and confirming email...');
